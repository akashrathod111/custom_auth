...
<div class="container">
    <div class="card mt-5">
        <h2 class="card-header"><i class="fa-regular fa-credit-card"></i> Product</h2>
        <div class="card-body">
...
<script type="text/javascript">
  $(function () {
    ...
    var table = $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('ajex-products.index') }}",
        columns: [
            {data: 'id', name: 'id'},
            {data: 'name', name: 'name'},
    ...
    $('body').on('click', '.showProduct', function () {
      ...
      $.get("{{ route('ajex-products.index') }}" +'/' + product_id, function (data) {
    ...
    $('body').on('click', '.editProduct', function () {
      var product_id = $(this).data('id');
      $.get("{{ route('ajex-products.index') }}" +'/' + product_id +'/edit', function (data) {
    ...
    $('body').on('click', '.deleteProduct', function () {
        ...
        $.ajax({
            type: "DELETE",
            url: "{{ route('ajex-products.store') }}"+'/'+product_id,
